...
public class AAMapper  extends Mapper <LongWritable,Text,Text,Text> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, 
        ...
        StringTokenizer itr2 = new StringTokenizer(text.substring(1), "\n");
        StringTokenizer itr3 = new StringTokenizer(text.substring(2), "\n");
        ...
        {
            frame1 = itr1.nextToken().toString();
            for (int i = 0; i < frame1.length() - 1;  i += 3)
            {
                
                if(codon2aaMap.containsKey(frame1.substring(i, i + 3)))     
                {
                    String protein = codon2aaMap.get(frame1.substring(i, i + 3));
                    context.write(new Text(protein), new Text("Frame1" + "_" + "1"));
                }
            }
        }
        ...
        String frame2 = itr2.nextToken().toString();
        
        for (int i = 0; i < frame2.length() - 2; i += 3)
        {
            if(codon2aaMap.containsKey(frame2.substring(i, i + 3)))
            {
                String protein = codon2aaMap.get(frame2.substring(i, i + 3));
                context.write(new Text(protein), new Text("Frame2" + "_" + "1"));
            }
        }        
        
        while(itr2.hasMoreTokens())
        {
            frame2 = itr2.nextToken().toString().substring(1);
            for (int i = 0; i < frame2.length()- 2; i += 3)
            {
                if(codon2aaMap.containsKey(frame2.substring(i, i + 3)))
                {
        ...
        String frame3 = itr3.nextToken().toString();
        
        for (int i = 0; i < frame3.length() - 2; i += 3)
        {
            if(codon2aaMap.containsKey(frame3.substring(i, i + 3)))
            {
                String protein = codon2aaMap.get(frame3.substring(i, i + 3));
                context.write(new Text(protein), new Text("Frame3" + "_" + "1"));
            }
        }  
        
        while(itr3.hasMoreTokens())
        {
            frame3 = itr3.nextToken().toString().substring(2);
            for (int i = 0; i < frame3.length() - 2; i += 3)
            {
                if(codon2aaMap.containsKey(frame3.substring(i, i + 3)))
                {
